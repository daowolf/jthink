...
public class SaleTopProductProcessor extends AbstractElementTagProcessor {
	private static final String TAG_NAME = "topProductList";// 标签名
	private static final String FIELD_CATEGORY = "categoryId";
	private static final String PAGE_SIZE = "pageSize";
	private static final String PAGE_NUM = "pageNum";
	public static final int PRECEDENCE = 200;

	public SaleTopProductProcessor(String dialectPrefix) {
	...
	protected void doProcess(ITemplateContext context, IProcessableElementTag tag,
		...
		if (StringUtils.isNotBlank(categoryIdStr)) {
			...
			PageInfo<?> pageInfo = this.selectByPageNumSize(queryRequest,
					() -> postService.queryTopPostsByTermId(PostsType.PRODUCT, categoryId.toString()));
			...
			queryRequest.setPageSize(Integer.parseInt(pageSize));
			PageInfo<?> pageInfo = this.selectByPageNumSize(queryRequest,
					() -> postService.queryTopPostsByTermId(PostsType.PRODUCT, null));
			structureHandler.setLocalVariable("pageInfo", pageInfo);
		}
