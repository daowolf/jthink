...
public class HotPostProcessor extends AbstractElementTagProcessor {
	private static final String TAG_NAME = "hotPostList";// 标签名
	...
	public HotPostProcessor(String dialectPrefix) {
	...
	protected void doProcess(ITemplateContext context, IProcessableElementTag tag,
		...
		if (StringUtils.isNotBlank(categoryIdStr)) {
			...
			PageInfo<?> pageInfo = this.selectByPageNumSize(queryRequest,
					() -> postService.queryHotPosts(categoryId.toString()));
			structureHandler.setLocalVariable("pageInfo", pageInfo);
		} else {
			PageInfo<?> pageInfo = this.selectByPageNumSize(queryRequest, () -> postService.queryHotPosts(null));
			structureHandler.setLocalVariable("pageInfo", pageInfo);
		}
