...
function createTermsTree(){
	$.getJSON("/system/cms/category/treeList?_d="+new Date().getTime(),function(obj){
	var data = obj.msg;
...
function refreshTree(){
	$termTree.reload({reloadUrl:"/system/cms/category/treeList?_d="+new Date().getTime(),reloadKey:'msg'});
}
function closeModal() {
    $("#termAddBtn").attr("name", "save");
    $formValidator.resetForm();
    $Jtk.closeAndRestModal("addTermModal");
    $("#template").val("postList");
}
function initUpload(){
...
$(function(){
	...
	$("#termAddBtn").click(function(){
       ...
       $termAddForm.find("input[name='parentId']").val(selPid)
        ...
        if (flag) {
	         var formDatas = $termAddForm.serialize();
            if (name === "save") {
                $.post("/system/cms/category/add", formDatas, function (r) {
                    if (r.code === 200) {
                        refresh();
                        closeModal();
                        $Jtk.n_success(r.msg);
            ...
            if (name === "update") {
                $.post("/system/cms/category/update", formDatas, function (r) {
                    if (r.code === 200) {
                        refresh();
                        closeModal();
                        $Jtk.n_success(r.msg);
