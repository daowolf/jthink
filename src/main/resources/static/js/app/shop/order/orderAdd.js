...
$(function () {
    ...
    $("#orderAddBtn").click(function () {
        ...
        $formValidator = $orderAddForm.validate();
        var txIds = $termTree.getSelectedID();
        $("#termIds").val(txIds);
        var flag = $formValidator.form();
        if (flag) {
	        var formDatas = $orderAddForm.serialize();
           //  var metas=[];
        ...
        //     formDatas+="&extarsStr="+JSON.stringify(metas);
            if (name === "save") {
                $.post("/system/shop/order/add", formDatas, function (r) {
                    if (r.code === 200) {
                        refresh();
                        closeModal();
                        $Jtk.n_success(r.msg);
            ...
            if (name === "update") {
                $.post("/system/shop/order/update", formDatas, function (r) {
                    if (r.code === 200) {
                        refresh();
                        closeModal();
                        $Jtk.n_success(r.msg);
    ...
    $("#addOrderModal .btn-close").click(function () {
        closeModal();
    });
})
function initUpload(){
...
function closeModal() {
    $("#orderAddBtn").attr("name", "save");
    $formValidator.resetForm();
    postEditor.setData("");
    $Jtk.closeAndRestModal("addOrderModal");
    $("#template").val("order");
}
/**
 * 下拉列表树初始化
