function updateTopic() {
    ...
    if (!selected_length) {
        $Jtk.n_warning('请勾选需要修改的优惠券类型！');
        return;
    }
    if (selected_length > 1) {
        $Jtk.n_warning('一次只能修改一个优惠券类型！');
        return;
    }
    var couponId = selected[0].id;
    $.post( "/system/shop/coupon/getCoupon", {"couponId": couponId}, function (r) {
        if (r.code === 200) {
            var $form = $('#coupon-add');
            $form.modal();
            var coupon = r.msg;
            ...
            $form.find("textarea[name='options']").val(coupon.options);
