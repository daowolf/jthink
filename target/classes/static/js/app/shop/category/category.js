        ...
        columns: [
            ...
            },
	        {
	            ...
	            events : {
	                ...
	                'click .term-product': function (e, value, row, index) {
	                    selPost(row.id);
	                },
...
function operateFormatter(value, row, index) {
    return [
        ...
        '<a  class="term-product btn btn-xs btn-default m-r-5" title="分类商品" data-toggle="tooltip"><i class="mdi mdi-comment-text-outline"></i></a>',
...
function selPost(termId){
	loadUrlToMain("/system/cms/post/selpost?postType=product&termId="+termId);
}
function delTerm(id){
	$Jtk.confirm({
		text:" 确定要删除?",
        ...
        $.post('/system/shop/category/delete', {"termIds": id}, function (r) {
            if (r.code === 200) {
                $Jtk.n_success(r.msg);
                refresh();
            } else {
...
function deleteTerms() {
    ...
    $Jtk.confirm({
        ...
        $.post('/system/shop/category/delete', {"termIds": ids_arr}, function (r) {
            if (r.code === 200) {
                $Jtk.n_success(r.msg);
                refresh();
            } else {
