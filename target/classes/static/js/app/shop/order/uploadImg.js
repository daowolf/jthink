var $uploadUtil=(function(){
    ...
    function removeFile( file ) {
	    var serverImgId = file.source.ruid;
        ...
        // 删除隐藏域
        removePics(serverImgId);
    }
    function removePics(imgId){
	 var $input_file = $wrap.find('input[type="hidden"]');
     if($input_file.val()){
	    var _pics = $input_file.val();
        var arrs = _pics.split(",");
        var index = arrs.indexOf(imgId);
        arrs.splice(index,1);
        $input_file.val(arrs.join(","));
     }
    }
	...
	function _getFileObject(filePathOrUrl, cb) {
	
	  _getFileBlob(filePathOrUrl, function (blob) {
	
	  cb(_blobToFile(blob, 'test.jpg'));
   ...
   return {
	uploader:uploader,
	getFileBlob:_getFileBlob,
	blobToFile:_blobToFile,
	getFileObject:_getFileObject
   }
})($);
